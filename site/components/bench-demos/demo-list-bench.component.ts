...
export class DemoListBench extends AbstractComponent {
  ...
  protected getHTML(): string {
    return `<div>
        <div class="fx mb_s">
          <button onclick="{{ root.updateMany() }}" class="${UI_KIT}_button ${UI_KIT}_button-s ${UI_KIT}_button-primary mr_s">{{ root.i18n$::rx.updateButton }}</button>
          <button onclick="{{ root.toggleActive() }}" class="${UI_KIT}_button ${UI_KIT}_button-s ${UI_KIT}_button-secondary">{{ root.i18n$::rx.toogleButton }}</button>
        </div>
